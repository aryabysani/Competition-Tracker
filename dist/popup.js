...
// ─── Entry Point ──────────────────────────────────────────────────────────────
async function init() {
    const result = await chrome.storage.local.get(['hasOnboarded', 'competitions', 'lastSynced']);
    const { hasOnboarded, competitions = [], lastSynced } = result;
    if (!hasOnboarded) {
...
function bindMain() {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'local' && (changes.competitions || changes.lastSynced)) {
            chrome.storage.local.get(['competitions', 'lastSynced'], (r) => {
                const { competitions = [], lastSynced } = r;
                renderMain(competitions, lastSynced);
            });
        }
    });
...
function renderMain(allComps, lastSynced) {
    ...
    document.getElementById('activeCount').textContent = String(totalDeadlines);
...
function tick() {
    ...
    document.querySelectorAll('.deadline-row').forEach(row => {
        ...
        if (valEl)
            valEl.textContent = formatCountdown(ms);
        ...
        if (badgeEl) {
            badgeEl.textContent = label.text;
            badgeEl.className = `dl-badge ${label.cls}`;
        }
    });
...
function renderSyncTime(lastSynced) {
    ...
    if (mins < 1)
        ...
        el.textContent = `Synced: ${mins}m ago`;
    else if (days < 1)
        el.textContent = `Synced: ${hrs}h ago`;
    else
        el.textContent = `Synced: ${days}d ago`;
}
// ═══════════════════════════════════════════════════════════════════════════
// SCREEN 3 — ADD FORM
// ═══════════════════════════════════════════════════════════════════════════
...
async function autoFillForm() {
    try {
        ...
        if (!tab?.url?.includes('unstop.com'))
            return;
...
function addDeadlineRow() {
    ...
    startDate.type = 'date';
    startDate.className = 'dl-entry-date dl-start-date';
    const startTP = buildTimePicker(12, 0, 'AM');
    startTP.classList.add('dl-start-time');
    startsRow.appendChild(startsLbl);
    startsRow.appendChild(startDate);
    startsRow.appendChild(startTP);
    // Ends row
    ...
    endDate.type = 'date';
    endDate.className = 'dl-entry-date dl-end-date';
    const endTP = buildTimePicker(11, 59, 'PM');
    endTP.classList.add('dl-end-time');
    endsRow.appendChild(endsLbl);
    endsRow.appendChild(endDate);
    endsRow.appendChild(endTP);
    entry.appendChild(headerRow);
    entry.appendChild(startsRow);
    entry.appendChild(endsRow);
...
async function saveManualComp() {
    ...
    const result = await chrome.storage.local.get(['competitions']);
    const competitions = result.competitions || [];
...
function getDeadlines(comp) {
    if (comp.source === 'manual') {
        ...
        return (comp.deadlines || []).flatMap(d => {
            ...
            if (d.datetime)
                return [{ label: d.label, datetime: d.datetime }];
