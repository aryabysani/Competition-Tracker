...
(async function () {
    ...
    const apiTypes = /hackathon/.test(segment) ? ['hackathon', 'competition', 'quiz'] :
    ...
    const allRounds = (extracted.rounds || [])
        ...
        start: r.start && !isNaN(new Date(r.start).getTime()) ? new Date(r.start).toISOString() : null,
        end: r.end && !isNaN(new Date(r.end).getTime()) ? new Date(r.end).toISOString() : null,
    }))
    ...
    const nextRound = allRounds.find(r => r.start && new Date(r.start).getTime() > now) ||
        allRounds[0] ||
        null;
    if (!nextRound) {
    ...
    function parseApiJson(json, type) {
        ...
        if (!name)
            return null;
        const host = comp.organisation?.name ||
        ...
        rawRounds.forEach((r, i) => {
            ...
            const roundName = r.title || r.name || r.round_name || r.stage_name ||
                r.details?.[0]?.title || r.details?.[0]?.name ||
                `Round ${i + 1}`;
            // Start / end: check top-level then details[0]
            const start = r.start_date || r.start_dt || r.starts_at || r.from_date ||
                r.details?.[0]?.start_date || r.details?.[0]?.start_dt || null;
            const end = r.end_date || r.end_dt || r.ends_at || r.to_date ||
                r.last_date || r.deadline ||
                r.details?.[0]?.end_date || r.details?.[0]?.end_dt || null;
            if (end)
                rounds.push({ name: String(roundName).slice(0, 120), start, end });
        });
        ...
        if (rounds.length === 0) {
            ...
            if (end)
                rounds.push({ name: 'Competition', start: start || null, end });
        }
    ...
    function extractFromNextData() {
        ...
        (function walk(node) {
            if (opportunity || !node || typeof node !== 'object')
                return;
        ...
        (function findRounds(node) {
            if (!node || typeof node !== 'object')
                return;
            if (Array.isArray(node)) {
                const looksLikeRounds = node.length > 0 && node.every(item => item && typeof item === 'object' && !Array.isArray(item) &&
        ...
        let isRegistered = null;
        for (const k of ['is_registered', 'isRegistered', 'user_registered', 'registered']) {
            if (opportunity[k] === true || opportunity[k] === 1) {
                isRegistered = true;
                break;
            }
        }
    ...
    function extractFromDom() {
        ...
        if (og)
            name = stripSiteSuffix(og.getAttribute('content') || '');
        if (!name)
            name = stripSiteSuffix(document.title);
        if (!name) {
        ...
        if (hostEl)
            host = hostEl.textContent.trim().slice(0, 120);
        ...
        if (rounds.length === 0) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                for (let k = i; k <= Math.min(lines.length - 1, i + 6); k++) {
                    ...
                    if (dm) {
                        end = parseLooseDate(dm[1]);
                        if (end)
                            break;
                    }
                }
                if (end)
                    rounds.push({ name: roundName, start: null, end: end.toISOString() });
            }
        }
    ...
    function extractFromStagesSection() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = i + 1; j <= Math.min(lines.length - 1, i + 6); j++) {
                const line = lines[j];
                if (SKIP_RE.test(line) || line.length < 4 || RANGE_RE.test(line))
                    break;
